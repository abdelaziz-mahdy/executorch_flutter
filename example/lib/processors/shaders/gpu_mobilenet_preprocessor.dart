...
/// GPU-accelerated MobileNet/ImageNet preprocessor using Flutter Fragment Shaders
///
...
/// - GPU-based resize with shortest side = 256
/// - Center crop to 224x224
/// - ImageNet normalization (mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
/// - Optimized single-loop tensor conversion
class GpuMobileNetPreprocessor extends ExecuTorchPreprocessor<Uint8List> {
  GpuMobileNetPreprocessor({required this.config});
  ...
  Future<void> _initializeShader() async {
    ...
    try {
      _program = await ui.FragmentProgram.fromAsset('shaders/mobilenet_preprocess.frag');
      _isInitialized = true;
  ...
  Future<List<TensorData>> preprocess(Uint8List input) async {
    try {
      ...
      throw PreprocessingException('GPU MobileNet preprocessing failed: $e', e);
    }
  }
  ...
  Future<TensorData> _imageToTensor(ui.Image image) async {
    ...
    // Note: The shader already applied ImageNet normalization
