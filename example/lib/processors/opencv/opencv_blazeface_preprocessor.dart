...
/// OpenCV-accelerated BlazeFace preprocessor
...
class OpenCVBlazeFacePreprocessor {
  ...
  Future<List<TensorData>> preprocess(Uint8List imageBytes) async {
    ...
    // Simple resize to target size (BlazeFace uses 128x128)
    ...
    // Using alpha = 1/127.5 and beta = -1.0 gives us: pixel * (1/127.5) - 1 = pixel/127.5 - 1
    ...
    // BlazeFace expects [1, 128, 128, 3] in NHWC format
    ...
    debugPrint(
      '📊 OpenCV BlazeFace Tensor shape: [1, ${config.targetSize}, ${config.targetSize}, 3] (NHWC)',
    );
    debugPrint(
      '📊 OpenCV async processed ${floats.length} floats, range [-1, 1]',
    );
