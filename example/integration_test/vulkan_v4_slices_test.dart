...
/// V4 re-exported MobileNet slices test.
///
/// Tests freshly exported MobileNet slices with current ExecuTorch source.
///
/// Run:
///   cd example
///   flutter test integration_test/vulkan_v4_slices_test.dart -d 56241FDCH00AA2
...
void main() {
  ...
  group('V4 MobileNet Slices (re-exported)', () {
    late List<Map<String, dynamic>> tests;

    setUpAll(() async {
      final s = await rootBundle.loadString(
        'assets/debug_models/v4_manifest.json',
      );
      final manifest = jsonDecode(s) as Map<String, dynamic>;
      tests = (manifest['tests'] as List).cast<Map<String, dynamic>>();
      print('Loaded ${tests.length} v4 test cases');
    });

    for (final sliceNum in [1, 2, 3, 4]) {
      testWidgets('mobilenet_slice_$sliceNum', (tester) async {
        final tc = tests.firstWhere(
          (t) => t['name'] == 'mobilenet_slice_$sliceNum',
        );
        ...
        print('\n${"=" * 70}');
        print('TEST: mobilenet_slice_$sliceNum');
