...
/// GPU-accelerated BlazeFace preprocessor using Flutter Fragment Shaders
///
...
/// - GPU-based simple resize to 128x128
...
class GpuBlazeFacePreprocessor extends ExecuTorchPreprocessor<Uint8List> {
  GpuBlazeFacePreprocessor({required this.config});
  ...
  Future<void> _initializeShader() async {
    ...
    try {
      ...
      _isInitialized = true;
      debugPrint(
        'GpuBlazeFacePreprocessor: GPU shader initialized successfully',
      );
    } catch (e) {
      debugPrint('GpuBlazeFacePreprocessor: Failed to load GPU shader: $e');
  ...
  Future<List<TensorData>> preprocess(Uint8List input) async {
    try {
      ...
      // Process on GPU (resize and normalize to [-1, 1])
  ...
  ///
  /// Note: The shader already applied [-1, 1] normalization, but the output
  /// is stored as normalized float values. We read the raw RGBA values and
  /// apply the normalization here since Flutter renders to 8-bit RGBA.
  Future<TensorData> _imageToTensor(ui.Image image) async {
    ...
    // Create float32 tensor in NHWC format [1, 128, 128, 3]
    // BlazeFace uses NHWC format (batch, height, width, channels)
    final totalPixels = config.targetSize * config.targetSize;
    final floats = Float32List(3 * totalPixels);

    // NHWC conversion with [-1, 1] normalization
    ...
    for (int i = 0; i < totalPixels; i++) {
      ...
      floats[i * 3] = pixels[pixelIndex] * scale - 1.0; // R channel
      floats[i * 3 + 1] = pixels[pixelIndex + 1] * scale - 1.0; // G channel
      floats[i * 3 + 2] = pixels[pixelIndex + 2] * scale - 1.0; // B channel
    }

    debugPrint(
      'GpuBlazeFacePreprocessor: Tensor shape: [1, ${config.targetSize}, ${config.targetSize}, 3] (NHWC)',
    );
